...
ExaVerif integrates three stages into a single pipeline.

1. **Scenario generation.** A developer describes the module's constraints in a YAML file. ExaVerif generates every valid constraint combination automatically.
2. **Verification execution.** Each combination is evaluated deterministically against pluggable backends (Spike, QEMU, Verilator). Results are collected as structured pass/fail records.
3. **Knowledge base feedback.** All results are committed to a shared knowledge store consumed by [Nexus](https://docs.ssccs.org/projects/nexus), the autonomous research infrastructure. Failure patterns become training material; coverage gaps become research targets; verified modules become reusable reference points for the entire RISC-V ecosystem.
...
The optional `--interpret` flag runs a lightweight LLM pass over failures and explains why each one occurred. All results are committed as permanent, queryable records in a shared knowledge store — a verifiable, append‑only surface accessible across the RISC‑V verification ecosystem. Once committed, these records become reusable verification knowledge for any project.

```{python}
#| label: fig-nexus-loop
#| fig-cap: "ExaVerif verification results flow into Nexus's knowledge base, where the agentic research loop extracts patterns and feeds refined strategies back into the verification engine."
dot("""
digraph VerificationLearningLoop {
    rankdir=LR
    node [shape=box, style=filled, fontname="Arial", fontsize=10, margin=0.35]

    Ev [label="ExaVerif\n• Constraint evaluation\n• Failure pattern detection\n• Coverage gap identification", fillcolor="#D4E6F1"]

    Nexus [label="Knowledge Base\n• Fact accumulation\n• Pattern extraction\n• Hint generation", fillcolor="#FADBD8"]

    Agent [label="SSCCS Research-PoC Loop\n• Hypothesis generation\n• Constraint refinement\n• Strategy optimization", fillcolor="#D5F5E3"]

    Ev -> Nexus [label="verification results\nfailure traces\ncoverage data", style=solid]
    Nexus -> Agent [label="training material\naccumulated facts", style=solid]
    Agent -> Ev [label="refined constraints\nimproved templates\nsmarter strategies", style=dashed, dir=back]
...
ExaVerif's verification results are consumed by [Nexus](https://docs.ssccs.org/projects/nexus), the autonomous research infrastructure that validates the same structural observation model ExaVerif is built on. Every constraint combination evaluated, every failure pattern discovered, and every coverage gap identified flows into Nexus's knowledge base, where it becomes training material for the agentic research loop and reusable knowledge for the entire RISC‑V ecosystem. This connection closes the loop between verification tooling and research infrastructure: the tool produces the data that the infrastructure learns from, and the infrastructure produces the insights that make the tool smarter.
...
ExaVerif is an independent domain entity built on the shared substrate — a working demonstration of the model described in the [SSCCS Project Model](https://docs.ssccs.org/projects). Every verification run feeds the shared knowledge store. A failure pattern discovered in one module becomes reusable knowledge for every other RISC‑V project.
